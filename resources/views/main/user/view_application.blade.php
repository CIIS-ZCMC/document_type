...
@section('subcontent')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        ...
        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                        ...
                        @php 
                        $value = \App\Models\Benefit::where(['id' => $benid])->first()
                        @endphp
                        @foreach($clientben->benefit_application_logs as $logs)
                            <label class="required"  for="phone"> {{$value->benefit_name}}</label>
                            <label class="required"  for="phone"> {{$logs->process_name}}</label>
                            <label class="required"  for="phone"> {{$logs->date}}</label>
